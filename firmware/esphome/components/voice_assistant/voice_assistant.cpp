...
// CS-159 fork addition: graceful end of the user's turn, see FORK_NOTES.md.
// Unlike request_stop() (which makes Home Assistant ABORT the pipeline: no
// transcript, no reply), this asks Home Assistant to close the audio stream and
// transcribe what it already has, then performs locally the same transition the
// STT_VAD_END event performs, so the microphone is released before playback.
